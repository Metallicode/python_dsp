...
def MakeFile(signal):
	...
	wavfile.write(f"melod{now.strftime('%d-%m-%Y %H-%M-%S')}.wav", 44100, signal)


#####FROM HERE IS THE DRIVER CODE

#FIRST WE SET SOME VARIABLES
steps_in_sequence = 100	#HOW MENY STEPS IN OUR MELODY
...
for i in range(steps_in_sequence):
	sequence += Make_A_Tone(signal_length, i*(base_freq+i*9)%700) #<----TRY PLAYING WITH THIS PART
